...
import net.xblacky.animexstream.utils.findBetween
...
class GogoCDN(override val server: VideoServer) : VideoExtractor() {
    ...
    override suspend fun extract(response: String): VideoContainer {
        ...
        if (url.contains("streaming.php")) {
            ...
            val it = document.select("script[data-name=\"episode\"]").attr("data-value")
            ...
            val file = (document.toString().findBetween("sources:[{file: '", "',"))
            if (file != null) {
                if (
                    try {
                        client.head(file)
